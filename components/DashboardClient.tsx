      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 text-sm">Adopted Trees</p>
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="px-6 py-4 bg-gradient-to-r from-sage-600 to-sage-700">
            <h2 className="text-xl font-bold text-white">📍 My Adopted Trees</h2>
          </div>

          {adoptions.length === 0 ? (
            <div className="p-6 text-center">
              <p className="text-gray-600 mb-4">You haven't adopted any tree yet</p>
              <Link href="/adopt/map">
                ...
                return (
                  <div key={adoption.id} className="p-6 hover:bg-gray-50 transition">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                      {/* Árbol Info */}
                      <div>
                        <p className="text-xs text-gray-500 mb-1">TREE</p>
                        ...
                        {typeof adoption.width === 'number' && (
                          <p className="text-xs text-gray-700">Width: {adoption.width} m</p>
                        )}
                        {typeof adoption.height === 'number' && (
                          <p className="text-xs text-gray-700">Height: {adoption.height} m</p>
                        )}
                        {adoption.root_zone && (
                      ...
                      <div>
                        ...
                        <div className="flex gap-2 flex-wrap">
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold text-white ${
                            ...
                            {adoption.status === 'adopted' ? '✅ Adopted' :
                             adoption.status === 'reserved' ? '⏳ Reserved' :
                             '📝 Available'}
                          </span>
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold text-white ${
                            ...
                            {adoption.payment_status === 'completed' ? '💳 Paid' :
                             adoption.payment_status === 'pending' ? '⏳ Pending' :
                             '❌ Error'}
                          </span>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="text-sm font-semibold text-gray-900">
                          {adoption.start_date ? new Date(adoption.start_date).toLocaleDateString('en-US') : 'N/A'}
                        </p>
                        <p className="text-xs text-gray-600 mt-1">
                          until {adoption.end_date ? new Date(adoption.end_date).toLocaleDateString('en-US') : 'N/A'}
                        </p>
                      </div>
                    ...
                    {treeReports.length > 0 && (
                      <div className="mt-4 border-t pt-4">
                        <h4 className="text-sm font-semibold text-gray-900 mb-3">📄 Reports for this tree ({treeReports.length})</h4>
                          ...
                          {treeReports.map((r) => (
                            <div key={r.id} className="p-3 bg-blue-50 border border-blue-200 rounded">
                              <div className="flex flex-wrap justify-between gap-2 mb-2">
                                ...
                                <div className="text-xs text-blue-600">{new Date(r.created_at).toLocaleDateString('en-US')}</div>
                              </div>
                  ...
                  .map(a => (
                    <div key={a.id} className="p-3 bg-yellow-50 border border-yellow-200 rounded">
                      <p className="text-sm font-semibold text-yellow-900">
                        {a.tree_name || `#${a.tree_id}`} expires on {a.end_date ? new Date(a.end_date).toLocaleDateString('en-US') : 'N/A'}
                      </p>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-3">
              <a href="/faq" className="block p-3 hover:bg-gray-50 border border-gray-200 rounded transition">
                <p className="text-sm font-semibold text-blue-600">How does adoption work?</p>
              </a>
              ...
              <Link href="/adopt/map">
                <button className="w-full p-3 hover:bg-gray-50 border border-gray-200 rounded transition text-left">
                  <p className="text-sm font-semibold text-blue-600">Adopt another tree</p>
                </button>
              </Link>
            </div>
          </div>
